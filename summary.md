...
| Worker launch and waves | `WORKER_PROMPT`, `build_prompt`, `prepare_worker`, `run_one_worker`, indexed `pick_wave`, `finalize_task` (stable result digest, lifecycle record, and cardinality-preserving changed-path attribution), `cmd_run`, `run_wave`. |
